...
**Taps:** Keep the bridge/GUI running so it holds one Proxmark session open. A brief wave should register quickly; the **laptop plays the Mario coin sound** immediately on a successful chip read. The GUI shows **racer name · bib · UUID**. The website still plays the Mario Kart sound when a lap is scored.
