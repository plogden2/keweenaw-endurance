...
4. **Proxmark3 path** — Hardware mode: `RFID_INJECT` off; use real write-tag API + continuous reader WebSocket. Single physical tag: rewrite participant UUID, then wait for authentic read before counting the lap.
...
- On due lap: `write-tag(participant)` → await real Proxmark read (timeout → issue) → assert UI/API lap increment.
- Serialize on the single tag (one write→read in flight).
...
- Prefer a dedicated seed name/ids for the compressed event so production AYCEB seed fixtures stay untouched, **or** a generator flag `--durations=30,15,5` that still matches fixture UUIDs if start times are always overwritten at runtime.
- Exact Playwright route-abort vs `context.setOffline` for “client cannot hit API” chosen in the plan to match how hosted sync is configured in local Docker.
- Proxmark write/read timeouts and retry policy tuned empirically on the attached reader during first run.
